...
Config.Jail = {
    ...
    jail = 'custom',                               -- Current options: 'qb' (For qb-prison) / 'r_prison' (For r_scripts prison: https://r-scripts.tebex.io/) / 'rcore' (For rcore-prison) / 'tk_jail' (For tk_jail) / 'hd_jail' (For hd_jail) / 'myPrison' (For myPrison) / 'qalle-jail' (For qalle-jail) / 'plouffe ' (For Plouffe Jail) / 'mx' (For mxJail) / 'custom' (For custom)
    BuiltInPrison = {
...
Config.RadarPosts = {           -- Radar posts for speed detection
    ...
    thresholds = {   -- Speed thresholds for fines
        [25] = 250, -- 30 over = $1000 fine
        [50] = 500, -- 40 over = $2000 fine
        [75] = 1000, -- 50 over = $5000 fine
        [100] = 2000, -- 50 over = $5000 fine
        [125] = 3000, -- 50 over = $5000 fine
    },
...
Config.GPSBlips = {
    ...
    sprites = {
        none = 185,      -- Blip for when not in a vehicle
...
Config.Locations = {
    LSPD = {
        blip = {
            enabled = true,
            coords = vec(-1852.62, -336.62, 44.01),
            sprite = 60,
            color = 29,
            scale = 1.0,
            string = 'WYPD'
        ...
        clockInAndOut = {
            ...
            coords = vec3(-1849.16, -343.66, 49.83), -- Location of where to go on and off duty(If not using target)
        ...
        bossMenu = {
            ...
            coords = vec3(-1861.31, -319.39, 34.71), -- Location of boss menu (If not using target)
    ...
    MRPD = {
        blip = {
            enabled = true,
            coords = vec(428.12, -978.38, 30.71),
            sprite = 60,
            color = 29,
            scale = 1.0,
            string = 'MRPD'
        ...
        clockInAndOut = {
            enabled = true,                        -- Enable clocking in and out at a set location? (If using ESX you must have a off duty job for each e.x. offpolice for police offsheriff for sheriff AND have grades for each pd grade - QBCORE REQUIRES NOTHING)
            jobLock = 'police',                    -- This must be set to which job will be utilizing (ESX MUST HAVE OFF DUTY JOB / GRADES FOR THIS - ex. offpolice or offsheriff)
            coords = vec3(440.33, -982.64, 30.69), -- Location of where to go on and off duty(If not using target)
            label = '[E] - Go On/Off Duty',        -- Text to display(If not using target)
            distance = 3.0,                        -- Distance to display text UI(If not using target)
            target = {
                enabled = false,                   -- If enabled, the location and distance above will be obsolete
                label = 'Go On/Off Duty',
                coords = vec3(440.33, -982.64, 30.69),
                heading = 91.06,
                distance = 2.5,
                width = 2.0,
                length = 1.0,
        ...
        bossMenu = {
            enabled = true,                        -- Enable boss menu?
            jobLock = 'police',                    -- Lock to specific police job? Set to false if not desired
            coords = vec3(461.32, -1007.27, 35.93), -- Location of boss menu (If not using target)
            label = '[E] - Access Boss Menu',      -- Text UI label string (If not using target)
            distance = 3.0,                        -- Distance to allow access/prompt with text UI (If not using target)
            target = {
                enabled = false,                   -- If enabled, the location and distance above will be obsolete
                label = 'Access Boss Menu',
                coords = vec3(461.32, -1007.27, 35.93),
                heading = 269.85,
                distance = 2.0,
                width = 2.0,
                length = 1.0,
        ...
        cloakroom = {
            enabled = false,                        -- WILL NOT SHOW IN QBCORE INSTEAD USE QB-CLOTHING CONFIG! Set to false if you don't want to use (Compatible with esx_skin & wasabi fivem-appearance fork)
            jobLock = 'police',                    -- Allow only one of Config.policeJob listings / Set to false if allow all Config.policeJobs
            coords = vec(451.35, -991.81, 30.69), -- Coords of cloakroom
            label = '[E] - Change Clothes',        -- String of text ui of cloakroom
            range = 2.0,                           -- Range away from coords you can use.
            uniforms = {                           -- Uniform choices
                [1] = {                            -- Order it will display
        ...
        personalLocker = {
            enabled = false,                       -- Enable personal locker for this station?
            jobLock = 'police',                    -- Job lock?
            coords = vec3(451.35, -991.81, 30.69), -- Area to prompt personal locker
            range = 2.0,                           -- Range it will prompt from coords above
            label = '[E] - Access Personal Locker',
            target = {
                enabled = false, -- If enabled, the location above will be obsolete
                label = 'Access Personal Locker',
                coords = vec3(451.35, -991.81, 30.69),
                heading = 354.94,
                distance = 2.5,
                width = 2.0,
                length = 1.0,
        ...
        evidenceLocker = {
            enabled = true,                      -- Enable evidence locker for this station?
            jobLock = 'police',                   -- Job lock?
            coords = vec3(478.13, -985.00, 24.91), -- Area to prompt personal locker
            range = 2.0,                          -- Range it will prompt from coords above
            label = '[E] - Access Evidence Locker',
            target = {
                enabled = true, -- If enabled, the location above will be obsolete
                label = 'Access Evidence Locker',
                coords = vec3(478.13, -985.00, 24.91),
                heading = 266.23,
                distance = 2.5,
                width = 2.0,
                length = 1.0,
