...
function ChoppingNotify(msg, msgType, length)
	if msg then
...
function ChoppingEmail(VehName, LicensePlate)
	...
	elseif Config.Phone == "jpr-phonesystem" then
		TriggerServerEvent('jpr-phonesystem:server:sendEmail', {
			...
			sender = Config.LangT["EmailHeader"], -- Submitted by
			event = {}, -- Optional 
		})
	...
	elseif GetResourceState("yseries") == 'started' then
		TriggerServerEvent("Pug:Server:SendyseriesMail", VehName, NewPlate)
	end
end

function TryToRemoveTracker()
	...
	elseif GetResourceState("qb-minigames") == 'started' then
...
local function camBehind(center4, backDist, upDist)
    local dir = headingToDir(center4.w)
    local base = vector3(center4.x, center4.y, center4.z)
    return base - (dir * backDist) + vector3(0.0, 0.0, upDist or 0.0)
end
...
function CallPoliceForCarJack(LicensePlate, model,Vehicle)
	...
	elseif GetResourceState("core_dispatch") == 'started' then
		...
		TriggerServerEvent("core_dispatch:addCall", 
			"10-99", --- @code string code of the call (like 10-19)
			"Vehicle Theft.", --- @message string message in the notification
			{
				{icon = "fa-venus-mars", info = gender} --- @info table of type {icon= string, info= string }
			},
			...
			431, --- @spritenumber Sprite that will be use on the minimap to
			3, --- @colornumber color use for the blip
			false --- @priority boolean define if the call is priority or not
		)
	elseif GetResourceState("codem-dispatch") == 'started' then
        local Data = {
            type = "Vehicle Theft.",
            header = 'Vehicle theft in progress.',
            text = 'Vehicle theft in progress.',
            code = "10-99",
        }
...
function FullyChoppedVehicleFunction(payy)
	-- Add whatever you want here for when a player finishes fully chopping a vehicle down and deletes it.
...
RegisterNetEvent("Pug_chopping:client:PlaySound", function(Sound, Volume)
	SendNUIMessage({
		action = "playSound",
		sound = Sound,
		volume = Volume
...
function HasItem(items, amount)
	...
	Config.FrameworkFunctions.TriggerCallback("Pug:server:GetPlayerHasItemChopping", function(HasItem)
		if HasItem then
			DoesHasItem = true
		else
			DoesHasItem = false
		...
		if Framework == "QBCore" then
			if Config.InventoryType == "ox_inventory" then
				...
				if Config.UsingNewQBCore then
					TriggerServerEvent("Pug:Server:OpenStashChopping", "warehouse_storage_"..CurrentGarage.bucketid, 1000000, 80)
				else
...
RegisterNetEvent("Pug:Client:CriminalGarageMenu", function(location)
	if location then location = GetEntityCoords(PlayerPedId()) end
...
-- OUTSIDE SPOTS LOOP (enter garage)
CreateThread(function()
    while true do
        local wait = 1000
        local ped = PlayerPedId()
        ...
        local insideAny, hitSpot = false, nil
        for id, spot in pairs(Config.GarageSpots) do
            local c2 = vector2(spot.center.x, spot.center.y)
            if #(p2 - c2) <= spot.radius then
                insideAny = true
                hitSpot = spot
                break
            end
        ...
        if insideAny then
            ...
            if not InSideBox then
                InSideBox = true
                GarageCoods.Zone.setplayercoords = hitSpot.center
                GarageCoods.Zone.cameracoords = camBehind(hitSpot.center, 3.5, 0.55)
                DrawTextChopping(Config.EnterTheGarageText)
                TriggerEvent("Pug:EnterGargeHitELoop", hitSpot.center)
            end
        else
            if InSideBox then
                InSideBox = false
...
-- INSIDE ZONE LOOP (manage/get-rid prompt)
local InsideLeaveGarage = false
local InSideLeaveBox = false
CreateThread(function()
    while true do
        local wait = 1000
        if InsideGarage then
            ...
            if IsPedInAnyVehicle(ped) and GetPedInVehicleSeat(GetVehiclePedIsIn(ped), -1) == ped then
                local pos = GetEntityCoords(ped)
				if #(pos - vector3(970.97, -2989.99, -39.65)) <= 7 then
					InsideLeaveGarage = true
				else
					if InsideLeaveGarage then
						InsideLeaveGarage = false
					end
				end
                if InsideLeaveGarage then
                    wait = 5
                    if not InSideLeaveBox then
						print(GarageCoods.Zone.setplayercoords,"GarageCoods.Zone.setplayercoords")
						if GarageCoods and GarageCoods.Zone and GarageCoods.Zone.setplayercoords then
							InSideLeaveBox = true
							DrawTextChopping(Config.EnterTheGarageText)
							TriggerEvent("Pug:EnterGargeHitELoopInside")
						end
                    end
                else
                    if InSideLeaveBox then
                        InSideLeaveBox = false
                        HideDrawTextChopping()
                    end
                end
            else
                if InSideLeaveBox or InsideLeaveGarage then
                    InSideLeaveBox = false
					InsideLeaveGarage = false
                    HideDrawTextChopping()
                end
            end
		elseif InsideLeaveGarage then
			InSideLeaveBox = false
			InsideLeaveGarage = false
			HideDrawTextChopping()
        end
...
RegisterNetEvent("Pug:Client:EnterCriminalGarage", function(data, First)
	if data.toggle then
		if Config.Debug then
			print'here1'
		end
		if data.info and data.info and data.info.garageloc and not Config.EnableEnterAtAllLocations and not First then
			local MyGarage = json.decode(data.info.garageloc)
			if #(GetEntityCoords(PlayerPedId()) - vector3(MyGarage.x, MyGarage.y, MyGarage.z)) > 10.0 then
				ChoppingNotify("This is not your garage", 'error')
				return
			end
		end
		CurrentGarage = data.info
		print(CurrentGarage,"CurrentGarage")
		if Config.Debug then
		...
		if GarageCoods ~= nil and not IsPedInAnyVehicle(PlayerPedId()) then
			...
			if Config.DoCameraEffect then
				...
				while #(GetEntityCoords(PlayerPedId()) - GarageCoods.Zone.cameracoords) >= 1.5 do
					Wait(100)
					if GetGameTimer() - startTime > 1000 then
						-- 3 seconds passed, stop waiting
						break
					end
				end
				DoScreenFadeOut(500)
		...
		if GarageCoods == nil then
			...
			StartPlayerTeleport(PlayerId(), GarageCoods['Zone']['setplayercoords'].x,GarageCoods['Zone']['setplayercoords'].y,GarageCoods['Zone']['setplayercoords'].z-1, GarageCoods['Zone']['setplayercoords'].w, true, true, true)
		end
		while IsPlayerTeleportActive() do Wait(100) end
		-- GarageCoods = nil
		CurrentGarage = nil
...
RegisterNetEvent("Pug:Client:CutScene", function(RouteBucket)
	...
	Config.FrameworkFunctions.TriggerCallback('Pug:server:HasCrimGarageCB', function(result)
        if result then
			...
			TriggerEvent("Pug:Client:EnterCriminalGarage", data, true)
		end
	end)
	local Ped = PlayerPedId()
	DoScreenFadeIn(1000)
...
RegisterNetEvent("Pug:FixCameraLoopWhileViewingUpgrade", function()
	...
	while true do
		...
		if ViewingUpgrade then
			if IsControlJustPressed(1, 51) or IsControlJustPressed(1, 177) or lib and not lib.getOpenContextMenu() then
				ViewingUpgrade = false
				DeleteTestPreviews()
				DestoryCamera()
				break
...
function GetGarageLocation()
	return GarageCoods['Zone']['setplayercoords']
end
...
function HasVehicleKeys(PlayerCar)
	local CarPlate = GetPlate(PlayerCar)
	if GetResourceState("wasabi_carlock") == 'started' then
		return exports.wasabi_carlock:HasKey(CarPlate)
	elseif GetResourceState("MrNewbVehicleKeys") == 'started' then
		return exports.MrNewbVehicleKeys:HasKeysByPlate(CarPlate)
	elseif GetResourceState("qs-vehiclekeys") == 'started' then
		return exports['qs-vehiclekeys']:GetKey(CarPlate)
	elseif GetResourceState("loaf_keysystem") == 'started' then
		return exports['loaf_keysystem']:HasKey(CarPlate)
	elseif GetResourceState("ak47_qb_vehiclekeys") == 'started' then
		return exports['ak47_qb_vehiclekeys']:HasKey(CarPlate)
	elseif GetResourceState("ak47_vehiclekeys") == 'started' then
		return exports['ak47_vehiclekeys']:HasKey(CarPlate)
	elseif Framework == "QBCore" then
		return exports["qb-vehiclekeys"]:HasKeys(CarPlate)
	else
		return true
	end
end
...
function GiveVehicleKeys(veh)
	local CarPlate = GetPlate(PlayerCar)
	if GetResourceState("wasabi_carlock") == 'started' then
		exports.wasabi_carlock:GiveKey(CarPlate)
	elseif GetResourceState("MrNewbVehicleKeys") == 'started' then
		exports.MrNewbVehicleKeys:GiveKeysByPlate(CarPlate)
	elseif GetResourceState("qs-vehiclekeys") == 'started' then
		exports['qs-vehiclekeys']:GiveKeys(CarPlate, veh, true)
	elseif GetResourceState("loaf_keysystem") == 'started' then
		exports['loaf_keysystem']:SetKeyUsage(veh)
	elseif GetResourceState("ak47_qb_vehiclekeys") == 'started' then
		exports['ak47_qb_vehiclekeys']:GiveKey(CarPlate)
	elseif GetResourceState("ak47_vehiclekeys") == 'started' then
		exports['ak47_vehiclekeys']:GiveKey(CarPlate)
	else
		TriggerEvent("qb-vehiclekeys", CarPlate)
	end
end
