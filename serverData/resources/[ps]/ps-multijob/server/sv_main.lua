...
RegisterNetEvent('QBCore:Server:OnJobUpdate', function(source, newJob)
    ...
    local isClockedIn = exports['drx_mdt']:GetOfficerClockedIn(source)
    -- if police job and not clocked in, clock them in and vice versa
    if (job == "police" or job == "sheriff") then
        if (onduty and not isClockedIn) or (not onduty and isClockedIn) then
            exports['drx_mdt']:ToggleClock(source, job)
        end
    end
end)