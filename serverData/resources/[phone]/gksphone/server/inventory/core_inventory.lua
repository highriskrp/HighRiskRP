...
print('[GKSPHONE] Core Inventory Loaded')
...
function SetItemData(source, item, data)
    ...
    if Config.MetaItem and Player then
        local inventory = 'content-' ..  Player:gsub(':','')
...
function SearchPhoneItems(source)
    ...
    if Player then
        local inventory = 'content-' ..  Player:gsub(':','')
        local item = exports['core_inventory']:getInventory(inventory)
