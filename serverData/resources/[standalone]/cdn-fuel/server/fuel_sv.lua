...
RegisterNetEvent("cdn-fuel:server:purchase:jerrycan", function(purchasetype)
    ...
    elseif purchasetype == "cash" then
        ...
        -- Ongeldig betalingstype
        TriggerClientEvent('QBCore:Notify', src, Lang:t("invalid_payment_type"), 'error')
        return
    end
    ...
    if Player.Functions.AddItem("jerrycan", 1, false, info) then
        ...
        TriggerClientEvent('QBCore:Notify', src, Lang:t("jerrycan_purchased"), 'success')
        TriggerClientEvent('inventory:client:ItemBox', src, QBCore.Shared.Items['jerrycan'], "add")
    else
        TriggerClientEvent('QBCore:Notify', src, Lang:t("failed_to_purchase_jerrycan"), 'error')
    end
end)
...
if Config.UseSyphoning then
    QBCore.Functions.CreateUseableItem("syphoningkit", function(source, item)
        ...
        if item.info.cdn_fuel == nil then
            item.info.cdn_fuel = '0'
    ...
    if Config.Ox.Inventory then
        if itemdata == "jerrycan" then
            if amount < 1 or amount > Config.JerryCanCap then
                if Config.FuelDebug then
                    print("Error, amount is invalid (< 1 or > " .. Config.JerryCanCap .. ")! Amount:" .. amount)
                end
                return
            end
        elseif itemdata == "syphoningkit" then
            if amount < 1 or amount > Config.SyphonKitCap then
                if Config.SyphonDebug then
                    print("Error, amount is invalid (< 1 or > " .. Config.SyphonKitCap .. ")! Amount:" .. amount)
                end
                return
            end
        end
        ...
        if ItemName ~= nil then
            ...
            if ItemName == 'jerrycan' or ItemName == 'syphoningkit' then
                local fuel_amount = tonumber(itemdata.metadata.cdn_fuel) or 0
                if type == "add" then
