...
function OpenInventory(source, identifier, data)
    ...
    print(source, QBPlayer.PlayerData.items, formattedInventory)
...
function AddItem(identifier, item, amount, slot, info, reason)
    ...
    -- Modified so it would dry in trunks and player inventory for now ~ Cyber
    -- if item == "wetbud" and type(identifier) == "string" and identifier:find('stash-')
    if item == "wetbud" and type(identifier) == "string" then
        if not info then
            info = {}
            info.time = os.time()
            info.dry = 0
