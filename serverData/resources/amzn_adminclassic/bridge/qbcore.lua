...
elseif Config.Framework == 'autodetect' then
    ...
    print('[^3INFO^7] ^3Auto-Detect^7: Detected ^3QB-Core^7 Framework.')
else
    return
end
...
if (IsDuplicityVersion()) then
    ...
    Bridge = {
        ...
        GetOfflinePlayerFromCharacterIdentifier = function(CitizenId)
            ...
            local MenuPlayerData = {
                id = 'OFFLINE', name = ('%s %s'):format(PlayerData.charinfo.firstname, PlayerData.charinfo.lastname), identifiers = PlayerData.citizenid, role = PlayerData.role, bank = ('$%s'):format(comma_value(PlayerData.money.bank)),
                ...
                accountid = PlayerData.charinfo.account, hunger = PlayerData.metadata['hunger'], thirst = PlayerData.metadata['thirst'], injail = PlayerData.metadata['injail'], lastonline = res.last_updated, amountofVehicles = amountofVehicles[1].count,
                ...
                duty = PlayerData.job.onduty and Lang:t('alerts.yes') or Lang:t('alerts.no'), gang = PlayerData.gang.label,
                gangrank = PlayerData.gang.grade.name, gangboss = PlayerData.gang.isboss and Lang:t('alerts.yes') or Lang:t('alerts.no'),
            ...
            for _, playerId in pairs(QBCore.Functions.GetPlayers()) do
                ...
                if Player then
                    ...
                    table.insert(PlayerList, {
                        id = playerId, name = GetPlayerName(playerId), identifiers = json.encode(identifiers), role = LoadedRole[playerId], bank = ('$' .. comma_value(Player.PlayerData.money.bank)) or 'Unknown',
        ...
        Notify = function(targetId, message) 
            TriggerClientEvent('QBCore:Notify', targetId, message)
        end,
        ...
        ClearPlayerInventory = function(targetId)
            ...
            elseif (Config.Inventory == 'autodetect' and GetResourceState('codem-inventory') == 'started') or Config.Inventory == 'codem-inventory' then
                ...
                local p = QBCore.Functions.GetPlayer(targetId)
                if p then p.Functions.ClearInventory() end
            end
        end,
        ...
        GetLeaderboardInfo = function() return Cache.MoneyLeaderboard, {} end,
        PlayerActions = {
            AddMoney = function(targetId, amount)
                local p = QBCore.Functions.GetPlayer(targetId)
                if p then p.Functions.AddMoney('cash', amount, 'Admin Action') end
            end, RemoveMoney = function(targetId, amount)
                local p = QBCore.Functions.GetPlayer(targetId)
                if p then p.Functions.RemoveMoney('cash', amount, 'Admin Action') end
            end, AddBank = function(targetId, amount)
                local p = QBCore.Functions.GetPlayer(targetId)
                if p then p.Functions.AddMoney('bank', amount, 'Admin Action') end
            end, RemoveBank = function(targetId, amount)
                local p = QBCore.Functions.GetPlayer(targetId)
                if p then p.Functions.RemoveMoney('bank', amount, 'Admin Action') end
            end
        }
    }
    ...
    RegisterNetEvent('919-admin:server:OpenPSInventory', function(targetId)
        local src = source
        
        -- Their own export was broken.
        ...
        if AdminPanel.HasPermission(src, 'openinventory') then
            CreateThread(function()
                ...
                exports['ps-inventory']:OpenInventory("otherplayer", targetId, nil, src)
            end)
        end
    end)

    RegisterNetEvent('919-admin:server:OpenNewQBInventory', function(targetId)
        local src = source
        if AdminPanel.HasPermission(src, 'openinventory') then
...
if (not IsDuplicityVersion()) then
    Bridge = {
        OpenInventory = function(targetId)
            ...
            elseif (Config.Inventory == 'autodetect' and GetResourceState('ps-inventory') == 'started') or Config.Inventory == 'ps-inventory' then
                TriggerServerEvent('919-admin:server:OpenPSInventory', targetId)
            elseif (Config.Inventory == 'autodetect' and GetResourceState('lj-inventory') == 'started') or Config.Inventory == 'lj-inventory' then
            ...
            elseif (Config.Inventory == 'autodetect' and GetResourceState('core_inventory') == 'started') or Config.Inventory == 'core_inventory' then
                ...
                TriggerServerEvent('919-admin:server:OpenNewQBInventory', targetId)
            end
        end, UncuffSelf = function() TriggerEvent('police:client:GetCuffed') end,
        Notify = function(message) QBCore.Functions.Notify(message) end,
    }
end
